...
bool check_available_direction(int d) {
 int f=1;
    if(d==3||d==1)
   {
       for(int i=0;i<N;++i)
    {
        for(int j=0;j<N;++j)
        {
            if(grid[i][j]==grid[i][j+1]&&grid[i][j]!=0)f=0;
        }
    }
   }
   if(d==5||d==2)
   {
     for(int j=0;j<N;++j)
    {
            for(int i=0;i<N;++i)
        {
            if(grid[i][j]==grid[i+1][j]&&grid[i][j]!=0)f=0;
        }
    }
   }
   int ff=0;
...
bool check_available_move(int d) {
    ...
    if (d == 3) res = check_available_direction(d);
   // rotate_90();
    //check direction down
    if (d == 5) res = check_available_direction(d);
   // rotate_90();
    //check direction left
    if (d == 1) res = check_available_direction(d);
   // rotate_90();
    //check direction up
    if (d == 2) res = check_available_direction(d);
   // rotate_90();
    return res;
}
...
bool check_valid_direction(int i) {
    if(i!=1&&i!=2&&i!=3&&i!=5)return false;
    else return true;
